...
class LGP(nn.Module):
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        pos_x, _ = QueryGroup(self.nsample, p, p, x, knn_idx, o, o,
                              local=True)  # (n, nsample, nsample, 3) (n, nsample, c)
        pos, x = pos_x
        ...
        pos_q, pos_k, pos_v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=self.num_heads),
                                  (pos_q, pos_k, pos_v))
...
class BasicBlock(nn.Module):
    def __init__(self, in_channel, out_channel, depth, num_heads, downsample=None, act_layer=nn.GELU,
        ...
        self.blocks = nn.ModuleList([
            SAPBlock(in_channel, num_heads, k=k, act_layer=act_layer, norm_layer=norm_layer, downscale=downscale,
...
class SAPFormer(nn.Module):
    def __init__(self, downscale, depths, channels, up_k, num_heads=[1, 2, 4, 8], \
                 drop_path_rate=0.2, num_layers=4, concat_xyz=False, cls_dim=64, final_dim=128, fea_dim=7, mid_dim=256,
                 num_classes=50, shape_classes=16, ratio=0.25, k=16, prev_grid_size=0.04, sigma=1.0, stem_transformer=False, use_cls=True):
        super().__init__()
        ...
        self.cls_map = nn.Sequential(
            nn.Linear(shape_classes, cls_dim),
        ...
        self.fea_embed = nn.Sequential(
            nn.Linear(cls_dim, mid_dim),
            nn.BatchNorm1d(mid_dim),
            nn.ReLU(inplace=True),
            nn.Linear(mid_dim, final_dim),
            nn.BatchNorm1d(final_dim),
            nn.ReLU(inplace=True),
        )

        # dim = channels[0] + cls_dim if use_cls else channels[0]
        dim = final_dim
    ...
    def forward(self, feats, xyz, offset, batch, neighbor_idx, cls):
        ...
        if self.use_cls:
            B = len(cls)
            N = feats.shape[0] // B
            cls_one_hot = torch.eye(self.shape_classes, device=feats.device)[cls, ] # one_hot, (B, shape_classes)
            cls_one_hot = self.cls_map(cls_one_hot)  # (B, cls_dim)
            cls_one_hot = cls_one_hot.repeat(1, N).reshape(B*N, -1)
            assert cls_one_hot.shape[0] == feats.shape[0], f'{cls_one_hot.shape} v.s. {feats.shape}'
            # feats = torch.cat((feats, cls_one_hot), dim=1)
            feats = feats + cls_one_hot

        feats = self.fea_embed(feats)
