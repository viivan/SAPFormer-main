...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis/s3dis_sapformer.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/s3dis_sapformer.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(rank, argss):
    ...
    if main_process():
        ...
        if args.wandb and args.debug == 0:
            wandb.init(
                    project="sapformer_s3dis_segmentation",
