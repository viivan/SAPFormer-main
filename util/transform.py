...
class CenterAndNormalize(object):
    def __init__(self, centering=True, normalize=True):
    ...
    def __call__(self, points, color):
        ...
        if self.normalize:
            m = np.max(np.sqrt(np.sum(points ** 2, axis=-1, keepdims=True)), axis=0, keepdims=True)
            points = points / m

        return points, color

class RandomDiffScale(object):
    def __init__(self, scale_low=0.8, scale_high=1.2):
        self.scale_low = scale_low
        self.scale_high = scale_high

    def __call__(self, points, color):
        # different scale for x,y,z seperately
        scale = np.random.uniform(self.scale_low, self.scale_high, 3)
        points[:, 0:3] = np.multiply(points[:, 0:3], scale)
        return points, color
...
class RandomScale_test(object):
    def __init__(self, scale_low=0.8):
        self.scale_low = scale_low
        # self.scale_high = scale_high

    def __call__(self, points, color):
        # scale = np.random.uniform(self.scale_low, self.scale_high)
        scale = np.ones(3)*self.scale_low
        points[:, 0:3] *= scale
        return points, color

    def __repr__(self):
        return 'RandomScale_test(scale_low: {}, scale_high: {})'.format(self.scale_low)
