...
def get_parser():
    parser = argparse.ArgumentParser(description='SAPFormer For Point Cloud Part Segmentation')
    parser.add_argument('--config', type=str, default='config/shapenetpart/shapenetpart_sapformer.yaml', help='config file')
    parser.add_argument('opts', help='see config/shapenetpart/shapenetpart_sapformer.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(rank, argss):
    ...
    if main_process():
        ...
        if args.wandb and args.debug == 0:
            wandb.init(
                    project="sapformer_shapenetpart",
